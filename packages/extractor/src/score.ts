...
import { hasPseudoState, normalizeSelector, selectorHasTag } from "./selectors";
import { getSemanticBoosts } from "./semantic";
import type { ColorCandidate, HeuristicWeights } from "./types";

// Load heuristic weights
const heuristicWeights = heuristicWeightsData as HeuristicWeights;
...
export const roleDesirability = (
	...
	// Danger: prefer red-orange hues with saturation
		...
		const inHueRange = hueRanges.some(([min, max]) => hue >= min && hue <= max);
...
export const matchTokens = (
	...
	const matches: Partial<Record<ColorPaletteKey, number>> = {};
...
export const scoreCandidate = (
	...
	const scores: Partial<Record<ColorPaletteKey, number>> = {};
	...
	if (!isButtonOrComponentSelector || !isBorderProperty) {
		...
		Object.entries(selectorMatches).forEach(([key, score]) => {
			scores[key as ColorPaletteKey] =
				(scores[key as ColorPaletteKey] || 0) + score;
		});
	}
	...
	if (varName?.match(/btn|button/)) {
		scores.primary = (scores.primary || 0) + SCORE_BOOST_BUTTON_PRIMARY;
	}
	if (varName?.match(/link/i)) {
	...
	if (varName) {
		...
		Object.entries(semanticBoosts).forEach(([key, boost]) => {
			scores[key as ColorPaletteKey] =
				(scores[key as ColorPaletteKey] || 0) + boost;
		});
	}
	...
	if (varName) {
		...
		if (isFrameworkVar) {
			...
			Object.keys(scores).forEach((key) => {
				scores[key as ColorPaletteKey] = Math.max(
					0,
					(scores[key as ColorPaletteKey] || 0) - SCORE_PENALTY_FRAMEWORK,
				);
			});
		}
	}
		...
		if (!isFocusRing && !isVisitedLink) {
			Object.keys(scores).forEach((key) => {
				if (key !== "ring") {
					scores[key as ColorPaletteKey] = Math.max(
						0,
						(scores[key as ColorPaletteKey] || 0) - PSEUDO_STATE_PENALTY,
					);
				}
			});
