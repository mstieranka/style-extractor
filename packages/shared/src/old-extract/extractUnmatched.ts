 ...
 */
export function extractUnmatchedColors(
	hexCodes: string[],
	isDarkMode = false,
): ThemePalette {
	...
	const existingSurface = neutrals.find(
		(n) => Math.abs(n.l - idealSurfaceL) < 0.03 && n !== bgObj && n !== textObj,
	);
	...
	const existingBorder = neutrals.find(
		(n) => Math.abs(n.l - idealBorderL) < 0.05 && n !== bgObj && n !== textObj,
	);
	...
	const existingMuted = neutrals.find(
		(n) => Math.abs(n.l - idealMutedL) < 0.1 && n !== bgObj && n !== textObj,
	);
