...
import { detectMode, hasPseudoState, isRule, isDeclaration } from "./selectors";
  ...
  const replaced = val.replace(
    ...
    (_match, varName: string, fallback: string) => {
      if (cssVariables.has(varName)) {
        const resolved = cssVariables.get(varName)!.value;
...
export const resolveMultiLevelColorVar = (
  ...
  if (depth > MULTI_LEVEL_VAR_MAX_DEPTH) return null;
...
export const resolveVarMode = (
  ...
  if (match && cssVariables.has(match[1])) {
    return cssVariables.get(match[1])!.mode;
  }
  return null;
};
