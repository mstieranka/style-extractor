...
import type { ThemePalette, ColorCandidate, CssVariablesMap } from "./types";
import {
  MIN_SCORE,
  MODE_WEIGHT_LIGHT,
  MODE_WEIGHT_DARK,
...
import { detectMode, isRule, isDeclaration } from "./selectors";
import {
  resolveValue,
...
import { getSemanticBoosts, extractSemanticRoles } from "./semantic";
 ...
 */
const createEmptyScores = (): Record<keyof ThemePalette, number> => ({
  background: 0,
  surface: 0,
  text: 0,
  muted: 0,
...
const addScoredCandidate = (
  ...
  scores: Partial<Record<keyof ThemePalette, number>>,
  candidateBase: {
    value: string;
  ...
  Object.entries(scores).forEach(([key, score]) => {
    ...
    perPurposeScores[key as keyof ThemePalette] = adjustedScore;
  });
  ...
  Object.entries(scores).forEach(([key, score]) => {
    ...
    if (adjustedScore > MIN_SCORE) {
      ...
      candidates[key as keyof ThemePalette].push({
        ...candidateBase,
        score: adjustedScore,
        ...perPurposeScores,
      });
...
export const collectCandidates = (
  ast: csstree.CssNode,
  cssVariables: CssVariablesMap,
): Record<keyof ThemePalette, ColorCandidate[]> => {
  ...
  cssVariables.forEach((varData, varName) => {
    ...
    const hasSemantic = Object.values(semanticBoosts).some((v) => v > 0);
