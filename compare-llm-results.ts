...
function loadPredictionRuns(filePath: string): RunResult[] {
  ...
  const toRunResult = (entry: unknown): RunResult => {
    ...
    return {
      kind: "valid",
      record: {
        color: (obj.color as TokenRecord["color"]) || ({} as TokenRecord["color"]),
        fontSize: (obj.fontSize as TokenRecord["fontSize"]) || ({} as TokenRecord["fontSize"]),
      },
    };
  };
...
function evaluateModel(modelName: string): {
  ...
  for (const testCase of TEST_CASES) {
    ...
    const predictionPath = resolve(`data/llm/${modelName}/${testCase.jsonFile}`);
    ...
    if (validRunCount === 0) {
      testCaseMetrics.push({
        ...
        minDeltaEScore: null,
        expectedFields: Object.values(expected.color).filter((v) => v !== null).length,
        presentFields: 0,
        preciseFields: 0,
        veryPreciseFields: 0,
        totalRuns,
...
function main() {
  ...
  for (const modelName of MODELS) {
    ...
    const tableData = testCaseMetrics.map((tc) => {
      const runsInfo = tc.totalRuns > 1
        ? ` (${tc.validRuns}/${tc.totalRuns} runs${tc.oomRuns > 0 ? `, ${tc.oomRuns} OOM` : ""})`
        : "";
      const pct = (n: number) =>
        tc.expectedFields > 0 ? ((n / tc.expectedFields) * 100).toFixed(1) + "%" : "N/A";
      return {
        "Test Case": tc.name + runsInfo,
        "Mean ΔE Score": tc.meanDeltaEScore !== null ? tc.meanDeltaEScore.toFixed(2) : "N/A",
        "Min ΔE Score": tc.minDeltaEScore !== null ? tc.minDeltaEScore.toFixed(2) : "N/A",
    ...
    const summaryData = [
      {
        ...
        Value: suiteMeanDeltaEScore !== null ? suiteMeanDeltaEScore.toFixed(2) : "N/A",
      },
      {
        Metric: "Min ΔE Score (across all cases)",
        Value: suiteMinDeltaEScore !== null ? suiteMinDeltaEScore.toFixed(2) : "N/A",
      },
      {
