...
async function main(): Promise<void> {
  ...
  try {
    ...
    const sheets = new Map<string, any>(); // styleSheetId -> header
    cdp.on("CSS.styleSheetAdded", (evt: any) => {
    ...
    const adoptedDump = await page.evaluate(() => {
      ...
      if (
        ...
        for (const s of (document as any)
      ...
      for (const el of Array.from(
        ...
        if (
          ...
          Array.isArray((sr as any).adoptedStyleSheets)
        ) {
          for (const s of (sr as any).adoptedStyleSheets as CSSStyleSheet[]) {
            out.push({
    ...
    const htmlAttrsCss = await page.evaluate(() => {
      ...
      for (const el of Array.from(bgColorElements)) {
        ...
        if (!bgcolor || !bgcolor.trim()) continue;
